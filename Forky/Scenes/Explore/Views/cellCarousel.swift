...
class cellCarousel: UICollectionViewCell {
    ...
    open var alphaMinimum: CGFloat = 0.3
    ...
    func setUI() {
        //self.contentView.addShadowWithOffset(opacity: 0.5, x: 0, y: 2.0)
    }
    
    override open func layoutSubviews() {
        super.layoutSubviews()
        mainView.clipsToBounds = true
        guard let carouselView = superview as? collCarouselExplore else { return }
    ...
    open func scale(withCarouselInset carouselInset: CGFloat) {
        ...
        if let collectionView = superview as? collCarouselExplore, collectionView.scrollDirection == .vertical {
        ...
        let alphaValue = percentageScale + alphaMinimum
